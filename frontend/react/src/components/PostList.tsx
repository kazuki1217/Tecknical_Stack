...
function PostList({ loggedInUserId, loggedInUserName }: { loggedInUserId: number | null; loggedInUserName: string | null }) {
  ...
  const [paginationMeta, setPaginationMeta] = useState<PaginationMeta>(INITIAL_PAGINATION_META); // ページ情報を管理
  ...
  return (
    <SidebarLayout loggedInUserName={loggedInUserName}>
      ...
      <div className="post-pagination" aria-label="投稿一覧のページ送り">
        <button className="post-pagination-button" type="button" onClick={() => movePage(paginationMeta.current_page - 1)} disabled={paginationMeta.current_page <= 1} aria-label="前のページ">
          <FaChevronLeft />
        </button>
        <span className="post-pagination-status">
          {paginationMeta.total === 0 ? "0件" : `${paginationMeta.from}-${paginationMeta.to} / ${paginationMeta.total}件`}
          <span className="post-pagination-page">ページ {paginationMeta.current_page} / {paginationMeta.last_page}</span>
        </span>
        <button className="post-pagination-button" type="button" onClick={() => movePage(paginationMeta.current_page + 1)} disabled={!paginationMeta.has_more_pages} aria-label="次のページ">
          <FaChevronRight />
        </button>
      </div>
    </SidebarLayout>

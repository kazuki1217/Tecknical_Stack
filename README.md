...
- GitHub Actions を用いて PHPUnit テストコードを自動実行する CI 環境と本番環境に自動デプロイする CD 環境を構築
- 死活確認ツール「UptimeRobot」を導入し、Nginx と Laravel API の稼働状況を定期的に確認・通知するように整備
- logrotate を導入し、ログファイルがストレージ全体の 5％ を超過しないように整備
- Mailpit を導入し、開発環境で外部送信を行わずメール内容を確認できる環境を整備
- Laravel において、ERROR レベル以上のログ発生時にメール通知するシステムを実装
- Codex：`AGENTS.md` と `SKILL.md` を作成し、Issue 作成・実装・検証・PR 作成までの開発フローをAIエージェントで効率化
- Claude：Codex と同様に `CLAUDE.md` と Skill を整備し、開発フローを効率化
